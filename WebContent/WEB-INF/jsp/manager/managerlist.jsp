...
<html>
  ...
  <body>
    ...
    <form action="manager/querymanager.html" method="post">
    	<table>
    		<tr>
    			<td>查询方式:<select name="searchType"><option value="Precise" <c:if test="${searchType eq 'Precise' }">selected="selected"</c:if>>精确查询</option><option value="Fuzzy" <c:if test="${searchType eq 'Fuzzy' }">selected="selected"</c:if>>模糊查询</option></select></td>
    	...
    	<div style="float:right"><input type="submit" value="查询"/></div>
    </form>
